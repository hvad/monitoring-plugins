...
def main():
    ...
    if percent >= critical:
        print('CRITICAL - Swap percentage usage : %2.1f%% Total Swap : \
              %s Free Swap : %s Used Swap : %s \
              |swap_percent =%s;%s;%s;0;100' % (percent, total,
              free, used, percent, warning, critical))
        raise SystemExit(2)
    elif percent >= warning:
        print('WARNING - Swap percentage usage : %2.1f%% Total Swap : %s \
              Free Swap : %s Used Swap : %s \
              |swap_percent =%s;%s;%s;0;100' % (percent, total,
              free, used, percent, warning, critical))
        raise SystemExit(1)
    else:
        print('OK - Swap percentage usage : %2.1f%% Total Swap : %s \
              Free Swap : %s Used Swap : %s \
              |swap_percent =%s;%s;%s;0;100' % (percent, total,
              free, used, percent, warning, critical))
        raise SystemExit(0)
